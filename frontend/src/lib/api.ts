...
export async function analyzeRoute(payload:object):Promise<Analysis>{
  const auth=token();
  const response=await fetch(`${apiBase()}/analysis/route`,{method:"POST",headers:{"Content-Type":"application/json",...(auth?{Authorization:`Bearer ${auth}`}:{})},body:JSON.stringify(payload)});
